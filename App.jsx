...
// Biblioteca de armas comuns (Capítulo de Equipamento) para adicionar com um clique
const WEAPONS_LIBRARY = [
  { nome: "Adaga de Bronze Celestial", dano: "1d4", tipo: "Perfurante", attr: "des", categoria: "Simples" },
  { nome: "Espada Curta", dano: "1d6", tipo: "Perfurante", attr: "des", categoria: "Simples" },
  { nome: "Espada Longa (Xifos)", dano: "1d8", tipo: "Cortante", attr: "for", categoria: "Marcial" },
  { nome: "Lança de Bronze", dano: "1d6", tipo: "Perfurante", attr: "for", categoria: "Simples" },
  { nome: "Machado de Batalha", dano: "1d8", tipo: "Cortante", attr: "for", categoria: "Marcial" },
  { nome: "Maça", dano: "1d6", tipo: "Contusão", attr: "for", categoria: "Simples" },
  { nome: "Arco Curto", dano: "1d6", tipo: "Perfurante", attr: "des", categoria: "Simples" },
  { nome: "Arco Longo", dano: "1d8", tipo: "Perfurante", attr: "des", categoria: "Marcial" },
  { nome: "Besta Leve", dano: "1d8", tipo: "Perfurante", attr: "des", categoria: "Simples" },
  { nome: "Escudo (investida)", dano: "1d4", tipo: "Contusão", attr: "for", categoria: "Simples" },
  { nome: "Ataque Desarmado", dano: "1", tipo: "Contusão", attr: "for", categoria: "Simples" },
];

// Biblioteca de equipamentos comuns
const EQUIPMENT_LIBRARY = [
  { nome: "Mochila", descricao: "Carrega itens de aventura." },
  { nome: "Corda (15m)", descricao: "Resistente, útil para escalar ou amarrar." },
  { nome: "Kit de Primeiros Socorros", descricao: "Estabiliza ferimentos leves em campo." },
  { nome: "Lanterna / Tocha", descricao: "Ilumina uma área ao seu redor." },
  { nome: "Cantil", descricao: "Guarda água potável." },
  { nome: "Saco de Dormir", descricao: "Para descansar ao relento." },
  { nome: "Pederneira e Aço", descricao: "Usado para acender fogo." },
  { nome: "Ração de Viagem (5 dias)", descricao: "Alimento não perecível para a jornada." },
  { nome: "Kit de Disfarce", descricao: "Cosméticos, roupas e adereços para se passar por outra pessoa." },
  { nome: "Néctar (dose)", descricao: "Bebida dos deuses; cura em pequenas quantidades, mas é perigosa para mortais." },
  { nome: "Ambrosia (porção)", descricao: "Alimento dos deuses; cura ferimentos graves, com risco em excesso." },
];

// Talentos comuns de referência rápida
const TALENTOS_LIBRARY = [
  { nome: "Atlético", categoria: "Geral", descricao: "Melhora deslocamento e desempenho físico em provas de atletismo." },
  { nome: "Duro na Queda", categoria: "Combate", descricao: "Reduz dano de quedas e recupera-se melhor de estados debilitantes." },
  { nome: "Reflexos de Combate", categoria: "Combate", descricao: "Permite reagir com um ataque de oportunidade adicional ou similar." },
  { nome: "Iniciado em Magia", categoria: "Mágico", descricao: "Concede acesso a um truque ou habilidade menor de outra Filiação." },
  { nome: "Observador", categoria: "Geral", descricao: "Bônus em Percepção e Investigação, nota detalhes que passam despercebidos." },
];
...
const LS_PREFIX = "semideuses-rpg::";
...
function EditableList({ title, icon, rows, columns, onChange, addLabel = "Adicionar", hint, presets }) {
    ...
    <div className="elist">
      ...
      {presets && presets.length > 0 && (
        <div className="preset-chips">
          {presets.map((p, i) => (
            <button type="button" key={i} className="preset-chip" onClick={() => add(p.data)}>
      ...
      <div className="elist-table">
        ...
        {rows.length === 0 && (
          <div className="elist-empty">Nenhum item ainda. Use os atalhos acima ou adicione manualmente.</div>
        )}
        {rows.map((row, idx) => (
          <div className="elist-row" key={row._id || idx}>
            {columns.map((c) => (
              <div key={c.key} className="elist-cell" data-label={c.label} style={colStyle(c)}>
                {c.type === "textarea" ? (
...
function AttacksTable({ rows, atributos, pb, onChange, onRoll, presets }) {
  const update = (idx, patch) => onChange(rows.map((r, i) => (i === idx ? { ...r, ...patch } : r)));
  const remove = (idx) => onChange(rows.filter((_, i) => i !== idx));
  const add = (prefill) =>
    onChange([...rows, { _id: uid(), nome: "", attr: "for", prof: true, extra: 0, dano: "", tipo: "", notas: "", ...(prefill || {}) }]);
  ...
  return (
    <div className="elist">
      <div className="elist-head">
        <div className="elist-head-title">
          <Sword size={18} />
          <h4>Ataques, Armas e Poderes</h4>
        </div>
        <button type="button" className="btn-ghost small" onClick={() => add()}>
          <Plus size={14} /> Adicionar ataque
        </button>
      </div>
      <p className="elist-hint">O bônus de ataque é calculado sozinho: modificador do atributo + bônus de proficiência (se marcado) + extra.</p>
      {presets && (
        <div className="preset-chips">
          {presets.map((p, i) => (
            <button type="button" key={i} className="preset-chip" onClick={() => add(p.data)}>
      ...
      <div className="elist-table">
        <div className="elist-row elist-row-head">
          <div className="elist-cell" style={{ flex: 2 }}>Arma / Poder</div>
          <div className="elist-cell" style={{ width: "78px", flexShrink: 0 }}>Atrib.</div>
          <div className="elist-cell" style={{ width: "56px", flexShrink: 0 }}>Prof.</div>
          <div className="elist-cell" style={{ width: "56px", flexShrink: 0 }}>Extra</div>
          <div className="elist-cell" style={{ width: "70px", flexShrink: 0 }}>Bônus</div>
          <div className="elist-cell" style={{ width: "90px", flexShrink: 0 }}>Dano</div>
          <div className="elist-cell" style={{ flex: 1 }}>Tipo</div>
          <div className="elist-cell" style={{ flex: 1.4 }}>Notas</div>
          <div className="elist-cell" style={{ width: 36, flexShrink: 0 }} />
        </div>
        {rows.length === 0 && (
          <div className="elist-empty">Nenhum ataque ainda. Use os atalhos acima para adicionar uma arma comum.</div>
        )}
        {rows.map((row, idx) => {
          const total = mod(atributos[row.attr || "for"]) + (row.prof ? pb : 0) + (Number(row.extra) || 0);
          return (
            <div className="elist-row" key={row._id || idx}>
              <div className="elist-cell" data-label="Arma / Poder" style={{ flex: 2 }}>
                <input type="text" value={row.nome || ""} onChange={(e) => update(idx, { nome: e.target.value })} />
              </div>
              <div className="elist-cell" data-label="Atributo" style={{ width: "78px", flexShrink: 0 }}>
                <select value={row.attr || "for"} onChange={(e) => update(idx, { attr: e.target.value })}>
                  {ATTRS.map((a) => (
                    <option key={a.key} value={a.key}>{a.abbr}</option>
                  ))}
                </select>
              </div>
              <div className="elist-cell center" data-label="Proficiente" style={{ width: "56px", flexShrink: 0 }}>
                <input type="checkbox" checked={!!row.prof} onChange={(e) => update(idx, { prof: e.target.checked })} />
              </div>
              <div className="elist-cell" data-label="Extra" style={{ width: "56px", flexShrink: 0 }}>
                <input type="number" value={row.extra ?? 0} onChange={(e) => update(idx, { extra: Number(e.target.value) })} />
              </div>
              <div className="elist-cell bonus-cell" data-label="Bônus (auto)" style={{ width: "70px", flexShrink: 0 }}>
                <strong>{fmtMod(total)}</strong>
                <button type="button" className="roll-dot inline" onClick={() => onRoll(row.nome || "Ataque", total)} title="Rolar ataque">
                  <Dices size={12} />
                </button>
              </div>
              <div className="elist-cell" data-label="Dano" style={{ width: "90px", flexShrink: 0 }}>
                <input type="text" value={row.dano || ""} onChange={(e) => update(idx, { dano: e.target.value })} />
              </div>
              <div className="elist-cell" data-label="Tipo" style={{ flex: 1 }}>
                <input type="text" value={row.tipo || ""} onChange={(e) => update(idx, { tipo: e.target.value })} />
              </div>
              <div className="elist-cell" data-label="Notas" style={{ flex: 1.4 }}>
                <input type="text" value={row.notas || ""} onChange={(e) => update(idx, { notas: e.target.value })} />
              </div>
              <div className="elist-cell elist-actions" style={{ width: 36, flexShrink: 0 }}>
                <button type="button" className="icon-btn danger" onClick={() => remove(idx)} title="Remover">
                  <Trash2 size={14} />
...
function SheetView({ hero, setHero, onBack, saveStatus, rolls, onRoll }) {
  ...
  const handleFiliacaoChange = (id) => {
    const newGod = GOD_MAP[id] || null;
    setHero((h) => {
      // remove as linhas automáticas do deus anterior (mantém as que o jogador editou manualmente)
      let habilidadesFiliacao = h.habilidadesFiliacao.filter((r) => !r._auto);
      let trProf = { ...h.trProf };
      let periciasProf = { ...h.periciasProf };
      let profArmas = h.profArmas;
      let profArmaduras = h.profArmaduras;
      if (newGod) {
        habilidadesFiliacao = [
          { _id: uid(), _auto: true, nome: `Dom — ${newGod.name}`, nv: "1", rank: "Passiva", custo: "—", tipo: "Passiva", descricao: newGod.dom },
          { _id: uid(), _auto: true, nome: `Assinatura — ${newGod.sig.name}`, nv: "1", rank: "Ativa", custo: "—", tipo: "Especial", descricao: newGod.sig.text },
          ...habilidadesFiliacao,
        ];
        newGod.saves.forEach((k) => (trProf[k] = true));
        newGod.skills.forEach((s) => {
          if (!periciasProf[s]) periciasProf[s] = 1;
        });
        profArmas = profArmas || newGod.weapons;
        profArmaduras = profArmaduras || newGod.armor;
      }
      return { ...h, filiacaoId: id, habilidadesFiliacao, trProf, periciasProf, profArmas, profArmaduras };
    });
  };
  ...
  return (
    <div className="sheet">
      {navOpen && <div className="sidebar-overlay only-mobile" onClick={() => setNavOpen(false)} />}
      ...
      <div className="sheet-main">
          ...
          <SectionCard title="Identidade" icon={<User size={18} />}>
            <div className="grid-2">
              ...
              <Field label="Filiação (Deus)">
                <select value={hero.filiacaoId} onChange={(e) => handleFiliacaoChange(e.target.value)}>
        ...
        <section className={`tab-panel ${tab === "combate" ? "" : "tab-hidden"}`}>
          ...
          <AttacksTable
            rows={hero.ataques}
            atributos={hero.atributos}
            pb={pb}
            onChange={(rows) => update({ ataques: rows })}
            onRoll={onRoll}
            presets={WEAPONS_LIBRARY.map((w) => ({
              label: w.nome,
              data: { nome: w.nome, dano: w.dano, tipo: w.tipo, attr: w.attr, prof: true, extra: 0, notas: w.categoria },
            }))}
          />
        ...
        <section className={`tab-panel ${tab === "poderes" ? "" : "tab-hidden"}`}>
          ...
          <EditableList
            ...
            hint={god ? "O Dom e a Assinatura já foram adicionados abaixo automaticamente. Acrescente as demais habilidades da Filiação conforme for evoluindo." : "Selecione uma Filiação na aba Identidade para o Dom e a Assinatura entrarem aqui sozinhos."}
            columns={[
        ...
        <section className={`tab-panel ${tab === "talentos" ? "" : "tab-hidden"}`}>
          <EditableList
            ...
            presets={TALENTOS_LIBRARY.map((t) => ({ label: t.nome, data: { nome: t.nome, categoria: t.categoria, descricao: t.descricao } }))}
            columns={[
        ...
        <section className={`tab-panel ${tab === "equipamento" ? "" : "tab-hidden"}`}>
          <div className="grid-2 align-top">
            ...
            <EditableList
              ...
              presets={EQUIPMENT_LIBRARY.map((it) => ({ label: it.nome, data: { nome: it.nome, descricao: it.descricao } }))}
              columns={[
                { key: "nome", label: "Nome", flex: 1 },
                { key: "descricao", label: "Descrição", type: "textarea", flex: 2 },
              ]}
      ...
      <div className={`roll-panel ${rollDrawerOpen ? "drawer-open" : ""}`}>
        <div className="roll-panel-head">
          <Dices size={14} /> Rolagens
          <button type="button" className="icon-btn only-mobile roll-panel-close" onClick={() => setRollDrawerOpen(false)}>
            <X size={15} />
          </button>
        </div>
        {rolls.length === 0 && <p className="roll-empty">Toque nos dados ao lado de perícias, TRs e ataques.</p>}
        {rolls.map((r) => (
      ...
      {rollDrawerOpen && <div className="sidebar-overlay only-mobile" onClick={() => setRollDrawerOpen(false)} />}
      <button type="button" className="fab-dice only-mobile" onClick={() => setRollDrawerOpen(true)} title="Ver rolagens">
        <Dices size={20} />
        {rolls.length > 0 && <span className="fab-badge">{rolls[0].total}</span>}
      </button>
    </div>
  );
}
...
.app-root *{ transition:border-color .15s ease, background-color .15s ease, box-shadow .15s ease, color .15s ease; }
.app-root ::-webkit-scrollbar{ width:10px; height:10px; }
.app-root ::-webkit-scrollbar-track{ background:var(--void2); }
.app-root ::-webkit-scrollbar-thumb{ background:var(--bronze); border-radius:20px; border:2px solid var(--void2); }
.app-root ::-webkit-scrollbar-thumb:hover{ background:var(--gold); }
...
/* ---------- HOME ---------- */
.home{ max-width:1140px; margin:0 auto; padding:0 20px 60px; }
.home-hero{ padding:70px 8px 34px; text-align:center; position:relative; }
.home-hero-inner{ max-width:600px; margin:0 auto; position:relative; }
.brand-crest{
...
.brand-eyebrow{ letter-spacing:.2em; font-size:11px; color:var(--gold); font-weight:600; }
.brand-title{
  font-family:'Cinzel',serif; font-size:44px; margin:12px 0 16px; font-weight:700;
  color: var(--parchment);
  letter-spacing:.01em;
}
.brand-sub{ color:var(--mist-dim); line-height:1.6; margin-bottom:28px; font-size:15.5px; }
...
.hero-card-top{ display:flex; align-items:center; justify-content:space-between; color:var(--gold); margin-bottom:10px;}
.hero-card-icon{
  width:30px; height:30px; border-radius:50%; display:flex; align-items:center; justify-content:center;
  background:var(--void3); border:1px solid var(--line); color:var(--gold-bright);
}
.hero-card-level{ font-size:11.5px; background:var(--void3); padding:3px 10px; border-radius:20px; color:var(--gold-bright); border:1px solid var(--line); font-weight:600; }
.hero-card-name{ font-family:'Cinzel',serif; font-size:18px; color:var(--parchment); margin:0 0 5px; font-weight:600; }
.hero-card-god{ font-size:13px; color:var(--mist-dim); margin:0; }
.hero-card-actions{ position:absolute; top:14px; right:14px; display:flex; gap:4px; opacity:0; transition:opacity .15s; }
.hero-card:hover .hero-card-actions{ opacity:1; }
.hero-card-new{
  display:flex; flex-direction:column; align-items:center; justify-content:center; gap:10px;
  border:1.5px dashed var(--line); color:var(--gold); background:transparent; font-weight:600; font-size:13.5px;
}
.hero-card-new:hover{ background:var(--void2); border-color:var(--gold); color:var(--gold-bright); }
.home-footer{ text-align:center; color:var(--mist-dim); font-size:11.5px; margin-top:56px; opacity:.65; }
.empty-state{ text-align:center; padding:70px 20px; color:var(--mist-dim); display:flex; flex-direction:column; align-items:center; gap:12px; }
.empty-state svg{ color:var(--gold); }
.empty-state h3{ color:var(--parchment); font-family:'Cinzel',serif; font-weight:600; margin:4px 0; font-size:19px; }

/* ---------- BUTTONS ---------- */
.btn-primary{
  display:inline-flex; align-items:center; gap:8px; background:var(--gold);
  color:#241c08; border:none; padding:10px 18px; border-radius:8px; font-weight:700; cursor:pointer; font-size:14px;
  letter-spacing:.01em;
}
.btn-primary.big{ padding:14px 28px; font-size:15.5px; border-radius:9px; }
.btn-primary:hover{ background:var(--gold-bright); }
.btn-ghost{
  display:inline-flex; align-items:center; gap:6px; background:var(--void3); border:1px solid var(--line);
  color:var(--mist); padding:8px 13px; border-radius:8px; cursor:pointer; font-size:13px; font-weight:500;
}
...
.sidebar-overlay{ position:fixed; inset:0; background:rgba(10,9,20,.6); z-index:39; backdrop-filter:blur(2px); }
.sidebar{
  width:236px; flex-shrink:0; background:var(--void2); border-right:1px solid var(--line);
...
.sidebar-hero{ display:flex; align-items:center; gap:10px; padding:8px 6px; background:var(--void3); border-radius:10px; border:1px solid var(--line); }
.sidebar-hero-icon{
  width:36px; height:36px; border-radius:50%; background:var(--void3); color:var(--gold-bright);
  display:flex; align-items:center; justify-content:center; border:1.5px solid var(--gold); flex-shrink:0;
}
.sidebar-hero-name{ font-family:'Cinzel',serif; font-size:14px; color:var(--parchment); line-height:1.25; }
.sidebar-hero-sub{ font-size:11px; color:var(--mist-dim); }
.tabs-nav{ display:flex; flex-direction:column; gap:3px; margin-top:12px; flex:1; }
.tab-nav-item{
  display:flex; align-items:center; gap:11px; padding:10px 11px; border-radius:8px; background:transparent;
  border:none; color:var(--mist-dim); text-align:left; cursor:pointer; font-size:13.5px; font-weight:500;
  border-left:2px solid transparent;
}
.tab-nav-item:hover{ background:var(--void3); color:var(--mist); }
.tab-nav-item.active{
  background:linear-gradient(90deg, rgba(201,162,77,.16), transparent); color:var(--gold-bright);
...
.card{
  ...
  border-radius:12px; padding:19px 21px; position:relative;
}
.card-head{ display:flex; align-items:center; gap:9px; color:var(--gold); }
.card-head h3{ font-family:'Cinzel',serif; font-size:16.5px; color:var(--parchment); margin:0; font-weight:600; letter-spacing:.01em; }
...
input[type=text], input[type=number], select, textarea, input:not([type]){
  background:var(--void); border:1px solid var(--line); border-radius:7px; padding:8px 11px;
  color:var(--parchment); font-size:13.5px; outline:none; width:100%;
}
textarea{ resize:vertical; font-family:'Cormorant Garamond',serif; font-size:15px; line-height:1.45; }
input::placeholder, textarea::placeholder{ color:var(--mist-dim); opacity:.6; }
input:hover, select:hover, textarea:hover{ border-color:var(--bronze); }
input:focus, select:focus, textarea:focus{ border-color:var(--gold); box-shadow:0 0 0 3px rgba(201,162,77,.13); }
...
.medallion-row{ display:grid; grid-template-columns:repeat(6,1fr); gap:16px; }
.medallion{ display:flex; flex-direction:column; align-items:center; gap:7px; position:relative; padding:10px 6px 6px; border-radius:12px; }
.medallion-ring{
  width:66px; height:66px; border-radius:50%; border:2px solid var(--gold); background:var(--void3);
  display:flex; align-items:center; justify-content:center;
  box-shadow: inset 0 0 0 3px var(--void2);
}
.medallion-mod{ font-family:'Cinzel',serif; font-size:21px; color:var(--gold-bright); font-weight:600; }
.medallion-abbr{ font-size:11px; letter-spacing:.14em; color:var(--mist-dim); font-weight:600; }
.medallion-score{ width:58px; text-align:center; padding:5px; font-weight:600; }
.medallion-prof{ display:flex; align-items:center; gap:4px; font-size:10.5px; color:var(--mist-dim); }
.medallion-label{ font-size:12px; color:var(--mist); font-weight:500; }
.roll-dot{
  position:absolute; top:4px; right:8px; width:23px; height:23px; border-radius:50%; background:var(--void3);
...
.roll-dot.inline{ position:static; width:21px; height:21px; }
...
/* ---------- EDITABLE LISTS ---------- */
.elist{ background:var(--panel); border:1px solid var(--line); border-radius:var(--radius); padding:16px 18px; max-width:100%; overflow-x:auto; }
.preset-chips{ display:flex; flex-wrap:wrap; gap:6px; margin-top:10px; }
.preset-chip{
  display:inline-flex; align-items:center; gap:4px; background:var(--void); border:1px dashed var(--line);
  color:var(--mist-dim); padding:5px 10px; border-radius:20px; font-size:11.5px; cursor:pointer;
}
.preset-chip:hover{ border-color:var(--gold); color:var(--gold-bright); border-style:solid; }
...
/* ---------- ROLL PANEL ---------- */
.roll-panel{
  width:226px; flex-shrink:0; border-left:1px solid var(--line); background:var(--void2); padding:18px 14px;
  position:sticky; top:0; height:100vh; overflow-y:auto;
}
.roll-panel-head{ display:flex; align-items:center; gap:7px; color:var(--gold); font-family:'Cinzel',serif; font-size:13px; margin-bottom:12px; letter-spacing:.03em; }
.roll-empty{ font-size:12px; color:var(--mist-dim); line-height:1.6; }
.roll-item{
  display:flex; flex-direction:column; gap:3px; padding:9px 10px; border-radius:8px; background:var(--void3); margin-bottom:7px;
  border:1px solid var(--line);
}
.roll-item.crit{ border-color:var(--gold); }
.roll-item.fumble{ border-color:var(--blood); }
.roll-label{ font-size:11.5px; color:var(--mist); font-weight:500; }
.roll-detail{ font-size:10.5px; color:var(--mist-dim); }
.roll-total{ font-family:'Cinzel',serif; font-size:19px; color:var(--gold-bright); align-self:flex-end; font-weight:700; }

.only-mobile{ display:none; }

.fab-dice{
  display:none; position:fixed; bottom:20px; right:20px; width:52px; height:52px; border-radius:50%;
  background:var(--gold); color:#241c08; border:none; align-items:center; justify-content:center;
  z-index:46; box-shadow:0 6px 18px rgba(0,0,0,.45); cursor:pointer;
}
.fab-badge{
  position:absolute; top:-4px; right:-4px; background:var(--void); color:var(--gold-bright);
  border:1.5px solid var(--gold); border-radius:20px; font-size:10.5px; font-weight:700;
  padding:1px 5px; min-width:18px; text-align:center;
}
.roll-panel-close{ margin-left:auto; }
...
@media (max-width: 760px){
  ...
  .app-root{ overflow-x:hidden; width:100%; }
  .only-desktop{ display:none; }
  .only-mobile{ display:flex; }
  .fab-dice{ display:flex; }
  .sidebar{
  ...
  .sheet-main{ padding:16px 14px 90px; max-width:100%; }
  ...
  .medallion-row{ grid-template-columns:repeat(3,1fr); gap:10px; }
  .hero-name-input{ font-size:24px; }
  .death-row{ flex-direction:column; gap:12px; }

  /* Painel de rolagens vira gaveta inferior no mobile */
  .roll-panel{
    position:fixed; left:0; right:0; bottom:0; top:auto; width:100%; height:auto; max-height:65vh;
    border-left:none; border-top:1px solid var(--line); border-radius:16px 16px 0 0; z-index:47;
    transform:translateY(110%); transition:transform .22s ease; box-shadow:0 -10px 30px rgba(0,0,0,.5);
  }
  .roll-panel.drawer-open{ transform:translateY(0); }

  /* Listas editáveis viram cartões empilhados no mobile, sem estourar a largura da tela */
  .elist-row-head{ display:none; }
  .elist-row{ flex-direction:column; gap:8px; background:var(--void3); border:1px solid var(--line); border-radius:10px; padding:10px 10px 6px; margin-bottom:8px; }
  .elist-cell{ width:100% !important; flex:none !important; }
  .elist-cell[data-label]::before{
    content:attr(data-label); display:block; font-size:10px; text-transform:uppercase; letter-spacing:.04em;
    color:var(--mist-dim); margin-bottom:3px;
  }
  .elist-cell.center{ flex-direction:row; align-items:center; gap:8px; }
  .elist-cell.center::before{ margin-bottom:0; }
  .elist-actions{ align-self:flex-end; padding-top:0; }
  .bonus-cell{ flex-direction:row !important; align-items:center; gap:8px; }
}

/* ---------- PRINT ---------- */
@media print{
  .sidebar, .roll-panel, .sheet-topbar, .storage-warning, .fab-dice{ display:none !important; }
